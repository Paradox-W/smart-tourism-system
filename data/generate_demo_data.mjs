...
function generateFoods(nodes, profile) {
  ...
  const count = Math.min(randInt(areaId + 500, 5, 10), allFoodNodes.length || 3)

  for (let f = 0; f < count; f++) {
    const nodeIdx = f % (allFoodNodes.length || 1)
    const node = allFoodNodes[nodeIdx] || nodes[Math.min(f + 5, nodes.length - 1)]
